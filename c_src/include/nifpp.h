...
// tanmay's enhancement
inline std::vector<double> ql_get_real_list(ErlNifEnv *env, ERL_NIF_TERM term)
{
  std::vector<double> firstParse;
  std::vector<int> fallback;
  try{
    get_throws(env, term, firstParse);
    return firstParse;
  }
  catch(...)
    {
      try{
        get_throws(env, term, fallback);
        std::vector<double> convert(fallback.begin(), fallback.end());
        return convert;
      }
      catch(...)
        {
...
inline double ql_get_double(ErlNifEnv *env, ERL_NIF_TERM term)
{
  double firstParse;
  int fallback;
  try {
